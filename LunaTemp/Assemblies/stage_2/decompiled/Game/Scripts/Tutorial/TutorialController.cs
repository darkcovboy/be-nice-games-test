...
namespace Game.Scripts.Tutorial
{
	public class TutorialController : MonoBehaviour
	{
		...
		[SerializeField]
		private RectTransform _holeMask;
		...
		private void StartTutorial()
		{
			...
			_overlayGroup.DOFade(1f, _fadeDuration);
		...
		{
			Vector3 screenPos = _mainCamera.WorldToScreenPoint(worldPos);
			_handPointer.position = screenPos + new Vector3(0f, -100f, 0f);
			_handPointer.DOAnchorPosY(_handPointer.anchoredPosition.y + 40f, 0.7f).SetLoops(-1, LoopType.Yoyo).SetEase(Ease.InOutSine);
		}
		...
		public void OnPlayerClickedStack()
		{
			if (!_clickedOnce)
			{
				...
				_handPointer.DOMove(_mainCamera.WorldToScreenPoint(_targetCellTransform.position), _moveDuration).SetEase(Ease.InOutSine);
			}
		}

		public void EndTutorial()
		{
			_overlayGroup.DOFade(0f, 0.5f).OnComplete(delegate
			{
